...
export default function Header() {
    ...
    const authenticatedNavItems: NavItem[] = [
        ...
        ...(user?.role !== "doctor" ? [{ href: "/doctors", label: "Doctors" }] : []),
            ...
            <div className="container mx-auto px-4 py-4">
                <nav className="flex justify-between items-center">
                    ...
                    <Sheet open={isOpen} onOpenChange={setIsOpen}>
                        ...
                        <SheetContent side="right" className="w-[300px] sm:w-[400px] bg-gray-800 bg-opacity-95 backdrop-blur-lg">
                            <nav className="flex flex-col h-full">
                                ...
                                <ul className="flex flex-col space-y-4">
                                    {user &&
                                        authenticatedNavItems.map((item) => (
                                            <li key={item.href}>
                                                <Link href={item.href} className="text-white hover:text-purple-300 transition-colors text-lg" onClick={() => setIsOpen(false)}>
                                                    {item.label}
                                                </Link>
                                            </li>
                                        ))}
