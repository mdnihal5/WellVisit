...
import { updateAppointment, cancelAppointment, deleteAppointment } from "@/lib/redux/features/appointmentSlice";
        ...
        return (
            <div className="flex gap-2 mt-4">
                {user?.role === "doctor" && !isCompleted && (
                    <>
                        <Button variant="outline" onClick={() => handleStatusUpdate(appointment._id, "completed")}>
                            Mark Completed
                        </Button>
                        <Button variant="outline" onClick={() => handleStatusUpdate(appointment._id, "cancelled")}>
                            Cancel
                        </Button>
                    </>
                )}

                {user?.role === "patient" && !isCompleted && (
                    <>
                        ...
                        <Button variant="outline" onClick={() => handleCancel(appointment._id)}>
                            Cancel
                        </Button>
                    </>
                )}

                <Button variant="destructive" onClick={() => handleDelete(appointment._id)}>
                    Delete
                </Button>
            </div>
        );
